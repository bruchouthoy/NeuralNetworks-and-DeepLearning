...
def cnn_3layer(x):
    weights  = {
        'W_conv1': tf.Variable(tf.random_normal([5, 5, 3, 64], stddev=0.1)),
        'W_conv2': tf.Variable(tf.random_normal([5, 5, 64, 128], stddev=0.1)),
        'W_conv3': tf.Variable(tf.random_normal([5, 5, 128, 256], stddev=0.1)),
        'W_fc1': tf.Variable(tf.random_normal([64*64*256, 1024], stddev=0.1)),
        'W_out': tf.Variable(tf.random_normal([1024, outputDim], stddev=0.1))
        }
    ...
    x = tf.reshape(x, shape=[-1, 512, 512, 3])
    # Convolution Layer 1, using our function
    ...
    flatten = tf.reshape(conv3, [-1, 64*64*256])
...
def train_neural_network():
    ...
    #Invoking the CNN model
    prediction = cnn_3layer(x)
    
    #softmax_cross_entropy with logits as the cost function
